...
import PurchaseForm from '../common/PurchaseForm';
...
const ProductDetailPage = () => {
  ...
  // Recommendation and map flow state
  const [showRecommendations, setShowRecommendations] = useState(true); // Show recommendations by default
  const [showMapAfterRecommendations, setShowMapAfterRecommendations] = useState(false);
  const [recommendedProducts, setRecommendedProducts] = useState([]);
  const [selectedProductsForMap, setSelectedProductsForMap] = useState([]);
  const [currentStep, setCurrentStep] = useState('product'); // 'product' -> 'maps' -> 'purchase'
  ...
  // Action handlers with recommendation flow
  const addToCart = () => {
    if (!product) return;
  ...
  const handleShopSelection = (shop, action = 'select') => {
    if (action === 'purchase') {
      setCurrentStep('purchase');
      setShowPurchaseForm(true);
      // Store selected shop for purchase
      setSelectedShop(shop);
    }
  };

  // Handle purchase completion
  const handlePurchaseComplete = (contactInfo) => {
    // Process purchase with selected shop and contact info
    console.log('Purchase completed:', {
      shop: selectedShop,
      products: selectedProductsForMap,
      contactInfo,
      quantity: qty
    });
    
    alert(`Order placed successfully! Confirmation will be sent to ${contactInfo.email || contactInfo.phone}`);
    setCurrentStep('product');
    setShowMapAfterRecommendations(false);
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '1rem' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <div style={{
            padding: '8px 16px',
            borderRadius: '20px',
            background: currentStep === 'product' ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.1)',
            fontWeight: currentStep === 'product' ? 'bold' : 'normal'
          }}>
          ...
          <div style={{
            padding: '8px 16px',
            borderRadius: '20px',
            background: currentStep === 'maps' ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.1)',
            fontWeight: currentStep === 'maps' ? 'bold' : 'normal'
          }}>
            2️⃣ Shop Locations
          </div>
          <div style={{ color: 'rgba(255,255,255,0.7)' }}>→</div>
          <div style={{
            padding: '8px 16px',
            borderRadius: '20px',
            background: currentStep === 'purchase' ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.1)',
            fontWeight: currentStep === 'purchase' ? 'bold' : 'normal'
          }}>
            3️⃣ Purchase
          </div>
        </div>
      ...
      <div style={{
        ...
        marginBottom: '1rem'
          ...
          <div style={{
            ...
            >
              {isOutOfStock ? 'Out of Stock' : '🛒 Add to Cart & Get Recommendations'}
            </button>
      ...
      {/* AI-Powered Recommendations Section - Always Visible Below Product */}
      {showRecommendations && (
        <div style={{
          border: '1px solid #e5e7eb',
          borderRadius: 10,
          ...
          <Recommendations 
            ...
            onRecommendationsLoad={handleGetMaps}
            showContinueButton={true}
          />
        </div>
      )}

      {/* Shop Availability Map Section */}
      {currentStep === 'maps' && showMapAfterRecommendations && (
        <div style={{
          border: '1px solid #e5e7eb',
          borderRadius: 10,
          padding: '1.5rem',
          ...
          <div style={{
            ...
            <h2 style={{ margin: 0, color: '#1e293b' }}>🗺️ Find Shops with Your Products</h2>
            <button
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 8px 0', fontWeight: 'bold', color: '#0c4a6e' }}>
              🎯 Finding shops with {selectedProductsForMap.length} product{selectedProductsForMap.length > 1 ? 's' : ''}
            </p>
            <p style={{ margin: 0, fontSize: '0.9rem', color: '#0369a1' }}>
              Select a shop from the map below to proceed with your purchase. We'll show you the nearest shops with the best availability.
            </p>
          </div>
          
          <ShopAvailabilityMap
            productIds={selectedProductsForMap}
            onShopSelect={handleShopSelection}
            userLocation={userLoc ? { lat: userLoc.latitude, lng: userLoc.longitude } : null}
      ...
      {currentStep === 'purchase' && showPurchaseForm && selectedShop && (
        <PurchaseForm
          shop={selectedShop}
          products={selectedProductsForMap}
          quantity={qty}
